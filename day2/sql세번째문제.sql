...
SELECT M_NAME, M_POINT, NVL(M_POINT,0) POINT
FROM MEMBER_TBL_11;

--문제) 힌트: NVL2함수를 사용하세요
--고객테이블로부터 기존포인트 점수에서 100포인트씩 인상하려고 합니다.  NEXT_POINT라는 컬럼으로 조회되도록 하시오.
--고객이름, 포인트와 함께 조회하시오 ( 기존포인트가 없는 사람은 0포인트로 간주합니다 
SELECT M_NAME, M_POINT, NVL(M_POINT+100, 0) NEXT_POINT
FROM MEMBER_TBL_11;
